...
export class ProductsService {
  ...
  getProducts(): Observable<Array<IProduct>> {
    ...
    return this.http
      .get<IGetAllProductsResponse>(
        `${this.api}/products?limit=0&skip=0&ordering=id`
      )
      .pipe(
  ...
  getProduct(id: number): Observable<IProduct> {
    return this.http
      .get<IGetProductResponse>(`${this.api}/product/${id}`)
      .pipe(map(res => res.product));
  }

  addProduct(product: IProduct): Observable<IProduct> {
    return this.http
      .post<IAddProductResponse>(`${this.api}/product`, product)
      .pipe(map(res => res.data));
  }

  updateProduct(product: IProduct): Observable<IProduct> {
    return this.http
      .patch<IUpdateProductResponse>(
  ...
  addProductImage(
    id: number,
    file: FormData
  ): Observable<HttpResponse<IAddProductImageResponse>> {
    return this.http.post<IAddProductImageResponse>(
      `${this.api}/product/${id}/image`,
      file,
      {observe: 'response'}
    );
  }
